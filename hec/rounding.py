...
class UsgsRounder:
    ...
    def round_s(self, value: float, round_half_even: bool=True) -> str:
        ...
        if abs(fraction - 0.5) < 1.e-8:
            if round_half_even and not integer % 2:
                pass
            else:
                integer += sign
        else:
            integer = int(value + .5 * sign)
    ...
    def rounding_spec(self, value) -> None:
        if not isinstance(value, str):
            raise TypeError(f"Expected str for 'rounding_spec', got {value.__class__.__name__}")
        if not value.isdigit() or len(value) != 10:
            raise ValueError(f"Expected a 10-digit string for 'rounding_spec', got '{value}'")
        self._rounding_spec = value

if __name__ == "__main__":
    rounder = UsgsRounder("4444444443")
    value = 1200567890.
    for i in range(14):
        print(f"{rounder.round_s(value)}\t{rounder.round_f(value)}")
        value /= 10.