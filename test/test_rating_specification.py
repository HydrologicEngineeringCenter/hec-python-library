...
def test_construct_name_only() -> None:
    spec = RatingSpecification("COUN.Count-Conduit_Gates,Opening-Conduit_Gates,Elev;Flow-Conduit_Gates.Standard.Production")
    assert spec.name == "COUN.Count-Conduit_Gates,Opening-Conduit_Gates,Elev;Flow-Conduit_Gates.Standard.Production"
    assert spec.location == Location("COUN")
    assert spec.template == RatingTemplate("Count-Conduit_Gates,Opening-Conduit_Gates,Elev;Flow-Conduit_Gates.Standard")
    assert spec.version == "Production"
    assert spec.lookup == default_lookup
    assert spec.rounding == (spec.template.ind_param_count + 1) * [DEFAULT_ROUNDING_SPEC]

def test_construct_bad_name() -> None:
    with pytest.raises(ValueError, match="Version cannot be an empty string"):
        spec = RatingSpecification("COUN.Count-Conduit_Gates,Opening-Conduit_Gates,Elev;Flow-Conduit_Gates.Standard.")
...
def test_construct_with_location() -> None:
    ...
    assert spec.template == RatingTemplate("Count-Conduit_Gates,Opening-Conduit_Gates,Elev;Flow-Conduit_Gates.Standard", office=location.office)
    assert spec.version == "Production"
    assert spec.lookup == default_lookup
    assert spec.rounding == (spec.template.ind_param_count + 1) * [DEFAULT_ROUNDING_SPEC]
...
def test_construct_with_template() -> None:
    template = RatingTemplate(
        ...
        lookup=[["error","error","error"],["linear","error","error"],["linear","error","null"]],
...
def test_construct_bad_template() -> None:
    with pytest.raises(ValueError, match="Elevation does not contain a recognized base parameter"):
        template = RatingTemplate(
...
def test_construct_bad_rounding() -> None:
    with pytest.raises(TypeError, match=re.escape("Expected 10-digit str for 'value[1]', got '123456789'")):
        spec = RatingSpecification(
            "COUN.Count-Conduit_Gates,Opening-Conduit_Gates,Elev;Flow-Conduit_Gates.Standard.Production",
