...
from hec.rating import LocalRatingSet
...
def test_local_rating_set_1_list(rating_set_1: LocalRatingSet) -> None:
...
def test_local_rating_set_1_ts(rating_set_1: LocalRatingSet) -> None:
    timestrs, counts, openings, elevations, expected_flows = list(
        map(list, zip(*rating_set_1_data))
    )
...
def test_local_rating_set_2_list(rating_set_2: LocalRatingSet) -> None:
    ...
    for t, e, v1, v2 in zip(timestrs, elevations, expected_stors, rated_stors):
        print(f"{t}\t{e}\t{v1}\t{v2}\t{np.isclose(v2, v1)}")
    assert np.allclose(expected_stors, rated_stors)


def test_local_rating_set_2_ts(rating_set_2: LocalRatingSet) -> None:
    timestrs, elevations, expected_stors = list(map(list, zip(*rating_set_2_data)))
    ...
    for i in range(num_each_time):
        ...
        rated_flows = rating_set_2.rate_time_series([elevations_ts], unit="ac-ft")
        assert np.allclose(expected_stors, rated_flows.values)


if __name__ == "__main__":
    with open("test/resources/rating/local_rating_set_1.xml") as f:
        xml = f.read()
    rs = LocalRatingSet.from_xml(xml)
    test_local_rating_set_1_ts(rs)
